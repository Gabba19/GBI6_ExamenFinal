...
```{r wd}
getwd() 
setwd("C:/Users/HILLARY BEBE/Documents/GBI6_ExamenFinal/GBI6_ExamenFinal")
```

### [0.5 PUNTOS] 1.2. Cargue la data "mRNA_expr.csv"
```{r data}

mRNA_expr <- read.csv2("~/GBI6_ExamenFinal/GBI6_ExamenFinal/data/mRNA_expr.csv")
df<- mRNA_expr
```
...
```{tex}
La tabla presenta los diferentes tipos de genes presentes en algunos cánceres, los datos describen los diferentes niveles de expresión, su media, rango pero se lo está representa en esta tabla dado la data es demasiado grande y los datos se vuelven difíciles de analizar.
```
...
Es una dispersión de la expresión de los genes relacionados yh la capacidad para mantener su expresión
